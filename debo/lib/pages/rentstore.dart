...
class RentStoreState extends State<RentStore> {
  ...
  _nestedScrollListener() async {
    ...
    if (nestedScrollController.position.pixels < 170) {
      await rentStoreProvider.setAppbarVisibility(true);
    } else if (nestedScrollController.position.pixels > 175) {
      await rentStoreProvider.setAppbarVisibility(false);
    }
  ...
  Future<void> setSelectedTab(int tabPos) async {
    ...
    if (!mounted) return;
    await rentStoreProvider.setSelectedTab(tabPos);
    printLog(
  ...
  Future<void> getTabData(int position) async {
    ...
    await rentStoreProvider.setRentLoading(true);
    await rentStoreProvider.setAppbarVisibility(true);
