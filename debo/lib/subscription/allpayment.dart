...
import 'package:debo/subscription/stripe/stripe_checkout.dart';
...
class AllPaymentState extends State<AllPayment> {
  ...
  Future addTransaction(
      pgName, packageId, description, amount, paymentId) async {
  ...
  Future addRentTransaction(pgName, videoId, amount, typeId, videoType) async {
    Utils.showProgress(context);
              ...
              children: [
                ...
                InkWell(
                  ...
                  onTap: () async {
                    ...
                    await paymentProvider.setFinalAmount(widget.price ?? "");
                    Utils.showToast(removeCouponMsg);
                  },
                  child: Container(
                    height: 30,
        ...
        children: [
          ...
          paymentProvider.paymentOptionModel.result?.razorpay != null
              ? paymentProvider
                      ...
                      "1"
                  ? _buildPGButton("pg_razorpay.png", "Razorpay", 35, 100,
                      onClick: () async {
    ...
    if (paymentProvider.paymentOptionModel.result?.razorpay != null) {
      ...
      var options = {
        'key': (paymentProvider.paymentOptionModel.result?.razorpay?.isLive ==
            ...
            : (paymentProvider.paymentOptionModel.result?.razorpay?.key4 ?? ""),
        'currency': Constant.currency,
  ...
  //     transactionCompleted: (PaymentData paymentData) async {
  //       printLog("Transaction Successful");
  //       printLog("paymentId ========> ${paymentData.paymentId}"); // استخدام paymentData بدلاً من paymentProvider
  //
  //       if (!mounted) return;
  //       Utils.showSnackbar(context, "success", "payment_success", true);
  //
  ...
  //             "paystack",
  //             paymentData.itemId,  // استخدام paymentData
  ...
  //             paymentData.finalAmount,
  //             paymentData.typeId,
  //             paymentData.videoType);
  //       }
  //     },
  //     transactionNotCompleted: (String errorMessage) async {
  //       printLog("Transaction Not Successful! Reason: $errorMessage"); // طباعة السبب
  //       if (!mounted) return;
  //       Utils.showSnackbar(context, "fail", "payment_fail", true);
  //     },
